...
static void prvInitialiseTCBVariables(TCB_t *const pxTCB, const char *const pcName, UBaseType_t uxPriority,
                                      ...
                                      const uint16_t usStackDepth) PRIVILEGED_FUNCTION; /*lint !e971 Unqualified char types are allowed for strings and single characters only. */
 ...
 */
static TCB_t *
prvAllocateTCBAndStack(const uint16_t usStackDepth, StackType_t *const puxStackBuffer) PRIVILEGED_FUNCTION;
  ...
  if ((pxTCB != NULL) && (pxTCB != pxCurrentTCB)) {
    taskENTER_CRITICAL();
    {
      if (prvTaskIsTaskSuspended(pxTCB) == pdTRUE) {
        ...
        (void) uxListRemove(&(pxTCB->xGenericListItem));prvAddTaskToReadyList(pxTCB);
...
BaseType_t xTaskRemoveFromEventList(const List_t *const pxEventList) {
  ...
  if (uxSchedulerSuspended == (UBaseType_t) pdFALSE) {
    (void) uxListRemove(&(pxUnblockedTCB->xGenericListItem));prvAddTaskToReadyList(pxUnblockedTCB);
  } else {
...
static TCB_t *prvAllocateTCBAndStack(const uint16_t usStackDepth, StackType_t *const puxStackBuffer) {
...
#else /* portSTACK_GROWTH */
  {
    ...
    /* Allocate space for the stack used by the task being created. */
    pxStack = (StackType_t *) pvPortMallocAligned((((size_t) usStackDepth) * sizeof(StackType_t)),
  ...
  if (pxMutexHolder != NULL) {
    ...
    if (pxTCB->uxPriority < pxCurrentTCB->uxPriority) {
      ...
      if (listIS_CONTAINED_WITHIN(&(pxReadyTasksLists[pxTCB->uxPriority]), &(pxTCB->xGenericListItem)) != pdFALSE) {
        ...
        pxTCB->uxPriority = pxCurrentTCB->uxPriority;prvAddTaskToReadyList(pxTCB);
      } else {
